...
`migrate inspect` and dry-run do not write project state. Apply writes backup,
staging, runtime state, event ledger, and journal data before replacing the
active Plan. Recovery rolls forward only the authenticated migration journal.
