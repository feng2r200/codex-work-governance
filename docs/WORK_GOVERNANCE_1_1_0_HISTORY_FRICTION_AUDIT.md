...
| Model repeatedly searched `workctl.py` to understand usage. | Improved, not eliminated | The generated CLI reference and `workctl help` are the supported entrypoints. `workctl.py` is now a thin public wrapper; the current kernel implementation is still large, so deeper implementation changes may still require code inspection. |
