...
# for direct runpy execution and runtime-bundle execution.
# ruff: noqa: E402, I001
...
def current_public_controller_path() -> Path:
    """Return the public controller script path for receipt identity checks."""
    argv0 = Path(sys.argv[0]).resolve()
    if argv0.name == "workctl.py":
        return argv0
    public_entrypoint = SCRIPT_DIR / "workctl.py"
    if public_entrypoint.is_file() and not public_entrypoint.is_symlink():
...
def validate_current_ready_receipt(
    ...
    *,
    ...
    if runtime_manifest != expected_manifest:
        raise WorkctlError("BOOTSTRAP_RUNTIME_BUNDLE_INVALID")
    if require_current_controller and current_public_controller_path() != controller:
