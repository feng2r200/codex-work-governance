...
updated_at: '2026-07-27T17:13:43+00:00'
scope:
  include:
...
- id: T-010
  ...
  resolves_artifacts:
  - A-007
  note: User confirmed the formal 1.0.0 promotion after corrected RC migration validation;
    only the exact version/cachebuster contract, local commit, reinstall, and fresh-session
    no-drift validation are authorized.
validations:
- id: V-001
...
- id: A-001
  ...
  state_evidence_ref: evidence:.work-governance/logs/PLAN-20260727-002/T-007-state-evidence-ref-deviation.md
...
- id: A-002
  ...
  state_evidence_ref: evidence:.work-governance/logs/PLAN-20260727-002/T-007-legacy-marker-deviation.md
...
route:
  ...
  next_phase: Request explicit confirmation before promoting the exact validated RC
    bytes to a formal 1.0.0+codex build and reinstalling it.
  validation_standard: Formal promotion must preserve the independently validated
    implementation bytes except for the exact version/cachebuster contract, reinstall
    from the local marketplace, open a fresh SessionStart, remain LAYOUT_READY and
    GOVERNED_ACTIVE without layout drift, and keep Push untouched.
  confirmation_gate: none
handoff:
  route_status: active
  next_step: Ask whether to promote the validated RC to the formal 1.0.0 build; do
    not change the installed version before that confirmation.
---
